...
public class RandomShapeGenerator<S extends Shape> implements RecordReader<Rectangle, S> {
    ...
    public RandomShapeGenerator(Configuration job,
        ...
        this(split.length,
                OperationsParams.getShape(job, "mbr") == null
                        ? new Rectangle(-90, -180, 90, 180)
                        : OperationsParams.getShape(job, "mbr").getMBR(),
    ...
    private static void generateShape(Shape shape, Rectangle mbr,
        ...
        if (shape instanceof HTMPoint) {
            generatePoint((HTMPoint) shape, mbr, type, random, circleThickness);
            Cartesian p = Latlon2Cartesian.parse(((HTMPoint) shape).x, ((HTMPoint) shape).y);
            ((HTMPoint) shape).HTMid = HTM.getInstance().Cartesian2HTMid(p, 20).getId();
