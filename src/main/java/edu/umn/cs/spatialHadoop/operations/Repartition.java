...
public class Repartition {
    ...
    /**
     * The map class maps each point to every top Trixels it falls inside.
     *
     * @author Haojia Zuo
     */
    public static class RepartitionMapHTM extends MapReduceBase
         ...
         * Used to output intermediate records
         */
        private IntWritable hidIndex = new IntWritable();
    ...
    public static class RepartitionReduceHTM extends MapReduceBase
            implements Reducer<IntWritable, HTMPoint, IntWritable, HTMPoint> {

        @Override
        public void reduce(IntWritable hidIndex, Iterator<HTMPoint> shapes,
                           OutputCollector<IntWritable, HTMPoint> output, Reporter reporter)
                throws IOException {
            HTMPoint shape = null;
            LOG.info("Closing partition #" + hidIndex);
            while (shapes.hasNext()) {
                shape = shapes.next();
                output.collect(hidIndex, shape);
            }
            LOG.info("Done with all records in #" + hidIndex);
            // Close cell
            output.collect(new IntWritable(-hidIndex.get()), shape);
            LOG.info("Done with HTMid #" + hidIndex);
        }
  ...
  */
    ...
    public static void repartitionMapReduce(Path inFile, Path outPath, CellInfo[] cellInfos,
        ...
        @SuppressWarnings("deprecation") final long blockSize = outFs.getDefaultBlockSize();
        ...
        if (sindex.equals("rtree") || sindex.equals("str")) {
            ...
            job.setMapperClass(RepartitionMapHTM.class);
        } else {
        ...
        // Set reduce function
        if (sindex.equals("htm")) {
            job.setReducerClass(RepartitionReduceHTM.class);
        } else {
    ...
    public static void repartition(Path inFile, Path outputPath,
                                   OperationsParams params) throws IOException, InterruptedException {
        if (params.get("sindex").equals("htm")) {
