...
public class OperationsParams extends Configuration {
    ...
    public static TextSerializable getTextSerializable(Configuration conf,
        ...
        try {
            ...
            if (shapeTypeI.startsWith("rect")) {
                ...
                shape = new Point();
            } else if (shapeTypeI.startsWith("htmpoint")) {
                shape = new HTMPoint();
    ...
    public static HTMRanges getRanges(Configuration conf, String key) {
        String raw = conf.get(key);
        String[] rangesRaw = raw.split(";");
        List<Pair<HTMid, HTMid>> pairList = new ArrayList<Pair<HTMid, HTMid>>();
        for (String rangeRaw : rangesRaw) {
            if (!rangeRaw.isEmpty()) {
                String[] idsRaw = rangeRaw.split(",");
                HTMid id1 = new HTMid(Long.parseLong(idsRaw[0]));
                HTMid id2 = new HTMid(Long.parseLong(idsRaw[1]));
                Pair<HTMid, HTMid> pair = new Pair<HTMid, HTMid>(id1, id2);
                pairList.add(pair);
            }
        }
        HTMRanges ranges = new HTMRanges(pairList);
        return ranges;
    }
    ...
    public static void setRanges(Configuration conf, String param, HTMRanges ranges) {
        String str = "";
        for (Pair<HTMid, HTMid> pair : ranges.getPairList()) {
            str += pair.a.getId() + "," + pair.b.getId() + ";";
        }
        conf.set(param, str);
    }
